// (c) Ger Versluis 2000 version 10.50 24 Februari 2004. You may use this script on non commercial sites. www.burmees.nl/menu

if(document.getElementById)
{
	var AgntUsr=navigator.userAgent.toLowerCase()
	var AppVer=navigator.appVersion.toLowerCase()
	var Opr7=AgntUsr.indexOf("opera 7")!=-1||AgntUsr.indexOf("opera/7")!=-1?true:false;
	if((AgntUsr.indexOf("opera")==-1||Opr7))
	{
		NavYes=((AgntUsr.indexOf("mozilla")!=-1&&AgntUsr.indexOf("compatible")==-1)||Opr7)?true:false,ExpYes=(AgntUsr.indexOf("msie")!=-1&&!Opr7)?true:false;
		...
		var Par=MenuUsesFrames?parent:window
...
function ChckInitLd()
{
	InitLdd = ExpYes?MenuUsesFrames?Par.frames[FirstLineFrame].document.readyState=="complete"&&Par.frames[SecLineFrame].document.readyState=="complete"?1:0:Par.document.readyState=="complete"?1:0:MenuUsesFrames?Par.frames[FirstLineFrame].document.body&&Par.frames[SecLineFrame].document.body?1:0:Par.document.body?1:0;
	if(InitLdd)
	{
		clearInterval(LdTmr);
		Go()
...
function RePos()
{
	FWinW=ExpYes?FCmplnt?FHtml.clientWidth:FLoc.document.body.clientWidth:FLoc.innerWidth;FWinH=ExpYes?FCmplnt?FHtml.clientHeight:FLoc.document.body.clientHeight:FLoc.innerHeight;
	SWinW=ExpYes?SCmplnt?ScHtml.clientWidth:ScLoc.document.body.clientWidth:ScLoc.innerWidth;SWinH=ExpYes?SCmplnt?ScHtml.clientHeight:ScLoc.document.body.clientHeight:ScLoc.innerHeight;
	if(MenuCentered.indexOf("justify")!=-1&&FirstLineHorizontal)
	{
		...
		var i
		var LftXtra=(NavYes&&!Opr7)||MacExp5||FCmplnt?LeftPaddng:0;
	...
	if(!Ldd)
	{
		...
		if(ScaleMenu)
		{
			if(document.defaultView&&document.defaultView.getComputedStyle)
				...
				if(ExpYes&&document.body.currentStyle)
		...
		if(ExpYes)
			FLoc.document.body.onunload=UnLddTotal;
...
function KeepPos()
{
	...
	if(TS!=StaticPos)
	{
		...
		FrstCntnr.style.top=FrstCntnr.OrgTop+P_X;StaticPos=TS
	}
}
...
{
	var a=BorderBtwnMain?NoOffFirstLineMenus+1:2
	var Sz=Math.round((PartOfWindow*FWinW-a*BorderWidthMain)/NoOffFirstLineMenus)
	var i,j;
...
function PosMenu(Ct,Tp,Lt)
{
	...
	var LftXtra=(NavYes&&!Opr7)||MacExp5||Cmplnt?LeftPaddng:0
	var TpXtra=(NavYes&&!Opr7)||MacExp5||Cmplnt?TopPaddng:0
	var Mb=Ct.FrstMbr
	var CStl=Ct.style
	var MStl=Mb.style
	var PadL=Mb.value.indexOf("<")==-1?LftXtra:0
	var PadT=Mb.value.indexOf("<")==-1?TpXtra:0
	var MWt=parseInt(MStl.width)+PadL
	var MHt=parseInt(MStl.height)+PadT
	var CWt=parseInt(CStl.width)
	var CHt=parseInt(CStl.height)
	var CCw, CCh, STp, SLt, Ti, Li, Hi
	var BRW=RLvl==1?BorderWidthMain:BorderWidthSub
	var BTWn=RLvl==1?BorderBtwnMain:BorderBtwnSub;
	
	if(RLvl==1&&AcrssFrms)
		!MenuFramesVertical?Tp=BottomUp?DistFrmFrameBrdr:FWinH-CHt-DistFrmFrameBrdr:Lt=RightToLeft?DistFrmFrameBrdr:FWinW-CWt-DistFrmFrameBrdr;
	if(RLvl==2&&AcrssFrms)
...
function GoTo()
{
	...
	if(this.Arr[1])
	{
		...
		IniFlg=true;
		Initiate();
		this.Arr[1].indexOf("javascript:")!=-1?eval(this.Arr[1]):DcLoc.location.href=BaseHref+this.Arr[1]
	}
}
...
function HiliteItem(P)
{
	...
	else
	{
		...
		if(HooverItalic)
			PS.fontStyle="italic";
		if(HooverUnderLine)
			PS.textDecoration="underline";
		if(HooverTextSize)
			PS.fontSize=parseInt(PS.fontSize)+HooverTextSize+P_X;
		if(HooverVariant)
			PS.fontVariant="small-caps"
	}
	P.Hilite=1
}
...
function LowItem(P)
{
	...
	else
	{
		...
		if(HooverItalic)
			PS.fontStyle="normal";
		if(HooverUnderLine)
			PS.textDecoration="none";
		if(HooverTextSize)
			PS.fontSize=parseInt(PS.fontSize)-HooverTextSize+P_X;
		if(HooverVariant)
			PS.fontVariant="normal"
	}
}
...
function OpenMenuII(Ptr)
{
	...
	{
		var Cmplnt=RLvl==1?FCmplnt:SCmplnt,LftXtra=(NavYes&&!Opr7)||MacExp5||Cmplnt?LeftPaddng:0
		var TpXtra=(NavYes&&!Opr7)||MacExp5||Cmplnt?TopPaddng:0;
		RLvl=Ptr.Lvl;
		Ptr.CCn=CreateMenuStructure(Ptr.MN+"_",Ptr.NofChlds,Ptr);
		var Ti, Li, Hi, MStl=Ptr.style, CCw, CCh, STp, SLt
		var PadL=Ptr.value.indexOf("<")==-1?LftXtra:0
		var PadT=Ptr.value.indexOf("<")==-1?TpXtra:0
		var MWt=parseInt(MStl.width)+PadL
		var MHt=parseInt(MStl.height)+PadT
		var BRW=RLvl==1?BorderWidthMain:BorderWidthSub;
		if(RightToLeft)
...
function Unfld()
{
	var P=CurOvr
	var PSt=P.style
	var CSt=P.Ctnr.style
	var CCnt=P.CCn
	var CCSt=CCnt.style
	var THt=parseInt(PSt.height)
	var TWt=parseInt(PSt.width)
	var TLt=AcrssFrms&&P.Lvl==1&&!FirstLineHorizontal?0:parseInt(CSt.left)
	var TTp=AcrssFrms&&P.Lvl==1&&FirstLineHorizontal?0:parseInt(CSt.top)
	var CCW=parseInt(CCSt.width),CCH=parseInt(CCSt.height)
	var TS=ExpYes?SCmplnt?ScHtml.scrollTop:ScLoc.document.body.scrollTop:ScLoc.pageYOffset
	var LS=ExpYes?SCmplnt?ScHtml.scrollLeft:ScLoc.document.body.scrollLeft:ScLoc.pageXOffset
...
function MbrSetUp(MbC,PrMmbr,WMu,Wd,Ht,Nofs)
{
	...
	if(this.Arr[6]&&!this.ro)
		TS.backgroundColor=this.Arr[6];
	TS.textAlign=this.Arr[15];
	if(TA2)
	{
		if(TA2.indexOf("rollover")==-1)
			TS.backgroundImage="url(\""+TA2+"\")";
		else
		{
			this.rob=1;
	...
	if(this.Arr[3])
	{
		...
		if(Arrws[a]!="")
		{
			T=RLvl==1&&FirstLineHorizontal?BottomUp?2:Ht-Arrws[a+2]-2:(Ht-Arrws[a+2])/2;
			L=RightToLeft?2:Wd-Arrws[a+1]-2;
...
function CreateMenuStructure(MNm,No,Mcllr)
{
	...
	var PMb=null
	var WMnu=MNm+"1"
	var MWd=eval(WMnu+"[5]")*FntFc 	// Menú Width (ancho)
	var MHt=eval(WMnu+"[4]")*FntFc	// Menú Height (alto)
	var Lctn=RLvl==1?FLoc:ScLoc
	var BRW=RLvl==1?BorderWidthMain:BorderWidthSub
	var BTWn=RLvl==1?BorderBtwnMain:BorderBtwnSub;
	if(RLvl==1&&FirstLineHorizontal)
	...
	for(i=1;i<No+1;i++)
	{
		WMnu=MNm+eval(i);
		NOs=eval(WMnu+"[3]");
		W=RLvl==1&&FirstLineHorizontal?eval(WMnu+"[5]")?eval(WMnu+"[5]")*FntFc:MWd:MWd;
		H=RLvl==1&&FirstLineHorizontal?MHt:eval(WMnu+"[4]")?eval(WMnu+"[4]")*FntFc:MHt;
