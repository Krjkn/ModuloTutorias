...
function nvo_encabezado_oficial($pdf)
{
	...
	$x_sep = 12;
	$y_sep = 0;
	$pdf->Image($CFG->imgLocalDir."/membrete.jpg", $x_sep, $y_sep, 74, 46);
	$pdf->Image($CFG->imgLocalDir."/tecnm.jpg", 116, 15, 84,13);
	...
	$pdf->SetXY(55, 25);
       // $pdf->Cell(100, 40, "''Año del Centenario de la Promulgación de la Constitución Política de los Estados Unidos Mexicanos''", 0, 2, 'C');
	/*$x = 12;
	$y = 52;
	$w = 1.7;
	$add = 0;
...
function nvo_encabezado_oficial_rh($pdf)
{
	...
	$pdf->SetFont('Helvetica','B','9');
	$pdf->SetXY(109, 71);
	$pdf->MultiCell(50, 4.5, "SECRETARÍA DE\nEDUCACIÓN PÚBLICA", 0, 'R', 0);
	$pdf->SetFont('Helvetica','','10');
	$pdf->SetXY(12, 68);
	$pdf->MultiCell(100, 4.9, "SUBSECRETARÍA DE EDUCACIÓN SUPERIOR\nDIRECCIÓN GENERAL DE EDUCACIÓN\nSUPERIOR TECNOLÓGICA", 0, 'L', 0);
	$pdf->SetLineWidth(0.5);
	$pdf->SetDrawColor(255, 204, 0);
...
function nvo_pie_oficial($pdf)
{
	...
	$y = 266;
	$x = 7;
        $pdf->Image($CFG->imgLocalDir."/escudo.jpg", 18, 272, 13);
	$w = 120;
	$h = 6;
	$x+=40+5;
	$pdf->SetXY($x-15, $y+4);
	...
	$pdf->Image($CFG->imgLocalDir."/unopie.jpg", 150, 272, 13);
	$pdf->Image($CFG->imgLocalDir."/dospie.jpg", 165, 270, 10,10);
	$pdf->Image($CFG->imgLocalDir."/trespie.jpg", 176, 270, 16);
	$pdf->SetLineWidth(0.1);
	$pdf->SetDrawColor(0);
}
...
$CFG = $GLOBALS['CFG'];
	$y = 260;
	$x = 7;
        $pdf->Image($CFG->imgLocalDir."/EscudoTec.jpg", 18, 262, 13);
	$w = 120;
	$h = 6;
	$x+=40+5;
	$pdf->SetXY($x-15, $y+4);
	...
	$pdf->Cell($w, $h/3, "", 0, 2, 'C');
	$pdf->Cell($w-15, $h/2, "Carretera Nacional Iguala Taxco esquina Periferico Norte, Col. Adolfo Lopez Mateos Infonavit, C.P. 40030", 0, 2, 'C');
	$pdf->Cell($w-15, $h/2, "Iguala, Gro. Tel(733) 3321425 Ext. 258, e-mail: escolares@itiguala.edu.mx", 0, 2, 'C');
	$pdf->Cell($w-15, $h/2, "Web ".$CFG->web, 0, 2, 'C');
	//$pdf->Image($CFG->imgLocalDir."/uno1.jpg", 150, 262, 13);
	$pdf->Image($CFG->imgLocalDir."/uno1.jpg", 165, 262, 13);
	$pdf->Image($CFG->imgLocalDir."/tres3.jpg", 179, 262, 13);
	$pdf->SetLineWidth(0.1);
	$pdf->SetDrawColor(0);
	...
	switch($depto)
	{
		case 'SE':
		{
			...
			if($tipo == 'P' || $tipo == '' || $tipo == '')
				...
				$asunto = "CONSTANCIA DE TÍTULO\nEN TRÁMITE";
			else if($tipo == 'I')
				$asunto = "CONSTANCIA DE ACREDITACIÓN\nDE LENGUA EXTRANJERA";
			else if($tipo == 'A')
				$asunto = "CONSTANCIA DE AVANCE";
			else if($tipo == 'I')
				$asunto = "CONSTANCIA DE TRÁNSITO\nESTUDIANTIL";
			else
				$asunto = "CONSTANCIA";
			break;
		}
...
function encabezado_documentos($pdf, $titulo)
{
	...
	$pdf->Image("img/sep.jpg", 10, 6, 43, 23);
	$pdf->Image("img/logo.jpg", 180, 6, 20, 20);
	$pdf->SetFont('Helvetica','b','16');
	$pdf->SetXY(60, 10);
	
}

function encabezado_documentosl($pdf, $titulo)
{
...
{
	$qry_alumno = "select * from pac_datosalumno('$no_de_control')";
	...
	/* Configuración de los datos del alumno */
	($accion=='boleta')?$pdf->SetFont('Helvetica','','10'):$pdf->SetFont('Helvetica','b','7');
...
function boleta($pdf, $no_de_control, $periodo, $accion=NULL)
{
	...
	$pdf->Image($CFG->imgLocalDir."/firma.jpg", 116, 200, 75,75);
	$pdf->Image($CFG->imgLocalDir."/valida.jpg",24 ,150 , 170, 50);
	$pdf->Image($CFG->imgLocalDir."/valida.jpg",24 ,50 , 170, 50);
	$pdf->Image($CFG->imgLocalDir."/valida.jpg",24 ,76 , 170, 50);
	$pdf->Image($CFG->imgLocalDir."/valida.jpg",24 ,125 , 170, 50);
	$pdf->Image($CFG->imgLocalDir."/valida.jpg",24 ,150 , 170, 50);
	...
	$var+= $wCr+0.5;
	$pdf->setXY($x+$var, $y);
	$pdf->MultiCell($wCa, $alto, "Calificación", 0, 'C', 1);
	...
	$qry_materias = "select * from  pac_historia_alumno('$no_de_control', '$periodo')";
	...
	$qry_extraescolares = "select * from  pac_histoextraescolar('$no_de_control', '$periodo')";
	...
	$var = $w+1.5;
	$pdf->setXY($x+$var, $y);
	$pdf->MultiCell($w, $alto/2, "Créditos\nCursados", 0, 'C', 1);
	...
	$var+= $w+0.5;
	$pdf->setXY($x+$var, $y);
	$pdf->MultiCell($w, $alto/3, "Promedio Aritmético del Periodo", 0, 'C', 1);
	
	$var+= $w+0.5;
	$pdf->setXY($x+$var, $y);
	$pdf->MultiCell($w, $alto/3, "Promedio Aritmético Acumulado", 0, 'C', 1);
	
	$non = true;
	$credacumapro = 0;	
	$qry_acumulado = "select * from  pac_datosacumuladohistorico('$no_de_control', '$periodo')";
	...
	$pdf->MultiCell(166, 2.5,"ESTE DOCUMENTO NO PODRÁ SER UTILIZADO COMO BOLETA OFICIAL DE CALIFICACIONES SI NO CUENTA CON LAS FIRMAS Y SELLO CORRESPONDIENTE", 0, 'C');
}
?>
