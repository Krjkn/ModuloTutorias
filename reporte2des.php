...
$qry_datos_grupo = "select departamento,CONCAT(nombre_empleado,' ',apellidos_empleado) as profesor,m.materia,g.grupo, 
	...
	$pdf->Cell(190,5,'Nombre del Tutor:  '.$profesor,1,0,'L');
	$pdf->Cell(30,5,'Fecha: '.$segundo,1,0,'L');
	$pdf->Cell(30,5,'No. de Reporte:  2 ',1,2,'L');
	$todayh=getdate();
	 $di = $todayh['mday'];
     $m = $todayh['mon'];
     $ye = $todayh['year'];
	...
	while ($fila5 = $res_lista->fetch_assoc()) {
		$alumno = $fila5['alumno'];
		$no_de_contro = $fila5['no_de_control'];
		$alumno = ' '.$i.' '.$no_de_contro.' '.$alumno;
		$pdf->Cell(120,5,$alumno,0,0,'L');
		$pdf->Cell(30,5,'X',0,2,'C');
		$pdf->SetX($x_inicial);
		$i++;
	}
	$pdf->SetXY($X1+150,$Y1);
	$qry_canaliza1 = "select no_de_control from seleccion_materias 
	where materia = '$materia' and grupo = '$grupo' and	periodo ='$periodo' order by no_de_control";
	$res_canaliza1 = ejecuta_consultaf($qry_canaliza1);
	while ($fila1 = $res_canaliza1->fetch_assoc()) {
		$no1 = $fila1['no_de_control'];
		$qry_canaliza2 = "select area,problema from canalizaciones where  '$no1' = no_de_control order by no_de_control";
	...
	where rfc = '$rfc' and periodo ='$periodo' and inicio <= '$segundo' and inicio >= '$primero' order by inicio";
	$res_calendario = ejecuta_consultaf($qry_calendario);
	$pdf->SetXY($x_inicial,$y_inicial+10);
	while ($fila = $res_calendario->fetch_assoc()) {
		$actividad = $fila['actividad'];
		$inicio = $fila['inicio'];
		$pdf->Cell(220,5,$actividad.'      '.$inicio,0,2,'L');
		$pdf->SetX($x_inicial);
	}
