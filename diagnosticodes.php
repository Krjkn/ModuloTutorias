...
$qry_datos_grupo = "select departamento,CONCAT(apellidos_empleado, ' ', nombre_empleado) as profesor,m.materia,g.grupo, 
	...
	$pdf->SetX($x_inicial);
	$pdf->Cell(190,5,"Carreras:  ".$nombre_carrera,1,2,'L');
	$pdf->SetX($x_inicial);
	...
	$Y1=$pdf->GetY();
	$pdf->SetXY($X,$Y);
	$pdf->Cell(190,5,utf8_decode('Problematíca Identificada:'),0,2,'L');
	
	$qry_canaliza = "select s.no_de_control,problema,objetivo,acciones from seleccion_materias s,canalizaciones c
	where s.materia = '$materia' and s.grupo = '$grupo' and	s.periodo ='$periodo' and s.no_de_control = c.no_de_control order by problema";
	...
	while ($fila = $res_canaliza->fetch_assoc()) {
		...
		$pdf->Cell(190,3,$problema.' '.$no_de_contro,0,2,'L');
	}
	$pdf->SetXY($X1,$Y1);
	$pdf->SetX($x_inicial);
	$X=$pdf->GetX();
	...
	while ($fila1 = $res_canaliza1->fetch_assoc()) {
		...
		$pdf->Cell(190,3,$objetivo.' '.$no_de_contro1,0,2,'L');
	}
	$pdf->SetXY($X1,$Y1);
	...
	while ($fila2 = $res_canaliza2->fetch_assoc()) {
		...
		$pdf->Cell(190,3,$acciones.' '.$no_de_contro2,0,2,'L');
	}
	...
	$pdf->SetXY($X,$Y);
	$pdf->SetFont('Helvetica','B','8');
	$pdf->MultiCell(95,2.7,$actividad,0,'L',0);
	$pdf->SetXY($X1,$Y1);
	$X=$pdf->GetX();$Y=$pdf->GetY();
	$pdf->SetFont('Helvetica','B','10');
	...
	while ($fila = $res_calendario->fetch_assoc()) {
		...
		$pdf->SetX($x_inicial);$pdf->Cell(190,5,$evaluacion,1,2,'L');
	}	
	$pdf->SetLineWidth(0.09);
	$pdf->SetFont('Helvetica','','10');
	$pdf->Line(125, 254,200,254);
	$pdf->SetXY(130,255);
	$pdf->Cell(70,5,$profesor,0,0,'C');
	$pdf->Output();
?>
