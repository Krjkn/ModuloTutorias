...
function abrir_documento(pagina, vtipo, vperiodo, vmateria, vgrupo,vmaestro,vviernes,vnmateria,vcarrera)
{
...
<form method="get" action="borraalumnogrupo.php">
	<table align="center">
    	<tr>
...
p.rfc = g.rfc and g.periodo= '$periodo' ";
	...
	while ($fila = $res_grupos->fetch_assoc()) 	
	{
		...
		$tabla_grupos_docente .= "<a><img src='img/lista.gif' onclick='abrir_documento(\"borra_alumno.php\", \" \", \"".$periodo."\", \"".trim($materia)."\", \"".trim($grupo)."\", \"".$profesor."\", \"".$viernes."\", \"".trim($nombre_completo_materia)."\", \"".trim($nombre_carrera)."\")' alt='Lista de Asistencia' style='cursor:pointer'/></a>";
