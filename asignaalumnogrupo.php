...
function abrir_documentoa(pagina, vtipo, vperiodo, vmateria, vgrupo)
{
...
<form method="get" action="asignaalumnogrupo.php">
	<table align="center">
    	<tr>
...
p.rfc = g.rfc and g.periodo= '$periodo' ";
	...
	$tabla_grupos_docente .= "		<th> Asigna alumno </th>";
	...
	while ($fila = $res_grupos->fetch_assoc()) 
	{
		...
		$tabla_grupos_docente .= "<img src='img/lista.gif' onclick='abrir_documentoa(\"inscribe_alumno.php\", \" \", \"".$periodo."\", \"".trim($materia)."\", \"".trim($grupo)."\", \"".$nombreC."\", \"".$viernes."\", \"".trim($nombre_completo_materia)."\", \"".trim($nombre_carrera)."\")' alt='Lista de Asistencia' style='cursor:pointer'/>";
